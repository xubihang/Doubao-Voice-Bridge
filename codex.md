...
架构决策:
DoubaoVoiceBridge 是编排层，只碰 macOS UI、事件 tap、焦点与进程生命周期。DoubaoVoiceBridgeCore 是纯策略与系统边界封装，承载配置、输入法、版本检测、权限报告和状态机。豆包 0.9.2 到 1.0.0 使用单次点按；1.0.1 及以后由触发键持续控制语音快捷键；更早或未知版本保留预热后长按。版本策略由 Core 统一判定，入口只消费策略结果。
...
2026-09-24 - 适配最新版豆包语音按住/释放行为；发送语音组合键 key-up 后等待 0.75 秒再恢复输入法；app bundle 版本提升到线上版本之后的 1.0.8。
